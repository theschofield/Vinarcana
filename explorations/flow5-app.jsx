// FLOW v5 — Reading override (fork of flow4-app): the desktop orbit keeps its
// float + cursor magnetism, and each orbit lens gets its roman numeral back —
// mirrored so the numeral rides the OUTER edge (mobile keeps numeral-left).
// Loads AFTER flow2-app.jsx and replaces Reading.
  ...
  React.useEffect(() => {
    ...
    const tick = (now) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (!el || !out) continue;
        const sd = ORB_SEEDS_V5[i % ORB_SEEDS_V5.length];
        let tx = 0, ty = 0, ts = 1;
        if (t.on && !reduced) {
...
function Reading({ card, light, whispered, flare, desktop, F, onPick, lensStep, spd, orbit }) {
  ...
  if (desktop) {
    const slots = c.lenses.length > 4 ? ORBIT_V5_5 : ORBIT_V5_4;
                ...
                <div className="orb-float" ref={(el) => { floatRefs.current[i] = el; }}>
                  ...
                  <div className="body">
                    <div className="namewrap">
                      <span className="rx-lens-num">{l.n}</span>
                      <span className="rx-lens-name">{l.name}{l.cellar ? <span className="rx-lens-dot"></span> : null}</span>
                    </div>
...
Object.assign(window, { Reading, useOrbitFloat });
