...
| D24 | Aug 21 | **THE RECKONING VERDICTS (Ed, answering docs/component-audit.md C10–C14 and docs/data-model.md C15–C22 — plain-language log; the audit docs carry the evidence).** (1) Taste sliders: ONE component everywhere, labels SOFT↔ACIDIC · DRY↔SWEET · SMOOTH↔TANNIC · LIGHT↔BOLD (the Pour's set; the Cellar detail switches to it). (2) Bottle image fallback until real images exist: red bottle for red wines, white bottle for everything else, on every screen (the Pour/Memory red default goes). (3) One wine page: the match screen, the detail, and S3's settled detail are STATES of one layout; the correction screen stays a list of options. (4) Pairing display types approved — THE BOTTLE / THE HOUSE / THE ARCHETYPE, mapped to §3.4's exact/producer/archetype; built at S4. (5) Migration order M1–M7 and its round sequencing approved (rack/detail polish rides the shared components). (6) Grapes on scanned wines are NOT taken from the label reader; S3's enrichment fills them (B-010 parked until S3). (7) The detail shows SUB-REGION and CLASSIFICATION whenever LWIN has them; DESIGNATION codes (AOP, AVA, DOCG…) are never shown — unintelligible to casual drinkers. (8) Wine names derive from LWIN's DISPLAY_NAME minus the producer, at RUNTIME in cellar-resolve (no index rebuilds, no maintenance); the raw WINE column is never displayed; long appellation names stay whole ("Beaune Premier Cru, Clos des Mouches Blanc"). (9) Orange/Dessert for scanned wines: no hand-kept list; waits for S3's style pass. (10) Drink window: shown ONLY for bottles in the cellar and ONLY when a 4-digit vintage is known — computed from vintage + type + region, never guessed (no window on the match sheet; an unreadable year stays unknown instead of becoming "NV"). Principle: wine-level facts are cached once per wine; bottle-level facts like the window are computed per bottle from its vintage. (11) Merchant rows ("Berry Bros & Rudd (Chateau Suduiraut)"): the producer line is the house in the parentheses; the outer name is kept as `bottler`. (12) Food pairings: NO per-wine tags, NO free-text PAIRS WITH rows; pairings are DERIVED from a wine's attributes by a pairing methodology (attributes → foods; any typed food → matching wines). New workstream, unscheduled; the curated PAIRS WITH rows leave the pours when it ships (content-pipeline task). (13) Bottle imagery: illustrated or generated bottles REJECTED for specific wines — real bottle photos are required, preferably via a retailer partnership traded for purchase deep-links; illustrated bottle SHAPES allowed only for archetype pairings ("Burgundy", "Riesling"). Deep research on sourcing ordered and DELIVERED the same day — docs/bottle-imagery-research.md (every claim sourced) with the readout in docs/data-model.md §4: no bottle-photo library exists for sale; the incumbents run on user-uploaded label photos; the strongest licensed source is wine.com's affiliate product feed (Rakuten Advertising — ~270k product pages, ~80% with studio bottle shots, image URL + tracked link per row), with UK retailer feeds and a direct Berry Bros licence as the second and third paths; the normalization pipeline is cheap and feasible offline. Next move is Ed's: the publisher sign-up + a written OK for in-app display. The build on these verdicts started the same day. |
| D23 | Jul 19 | **S2 device round 1 (COMPLETE).** Kill-switch drill PASSED (Ed tripped a guard limit naturally mid-round — the degrade to manual felt whole on device, the designed failure mode confirmed; the env-var flip itself is now a formality). Camera add, library add, forced miss → correction → manual, duplicate prompt, and identify-cancel all PASS on device. Copy verdicts SHIPPED same day: offline honest line → "No connection to your cellar right now"; the identify-fail CTA → "Manual entry" (the form headline keeps "Your bottle, your words."); third identify step "Gathering its story" → **"Pulling its record"** (session's proposal, Ed may re-word — the old step promised S3's story and read untruthful against the D11 sparse sheet, Ed's observation). va_quota SQL RUN — durable per-install quotas live. GOLDEN SET populated (10 fixtures, Ed's real bottles) and run 1 executed: 9/10 clean, and the misses taught more than the hits — Chanson Clos des Mouches IS in LWIN but Liv-ex's own WINE field is literally "Rouge"/"Blanc" with the appellation in other columns (B-015's smoking gun; explains Ed's first "Blanc" scan); Fieuzal's colour misread flips the type-agreement penalty against the CORRECT candidate (B-018); a conf-0.3 hallucinated read resolved to 0.69 — nearly the sheet bar — and a merchant row outranked Château Suduiraut (B-019). All three feed §5.8 checkpoint 1; the D22 audit and its kickoff are UNCHANGED. |
| D22 | Jul 19 | **THE RECKONING DIRECTIVE (Ed, after the first real scans — the pipeline works; the prototype's seams show).** Live scans matched real bottles, and exposed the hand-curated era's debts. Ed's course, agreed: a dedicated READ-ONLY audit session before further building — (1) COMPONENT AUDIT: visually-similar surfaces were built as separate JSX and are already diverging (cellar tile ⇄ match hero ⇄ detail hero; the wine-page family should be states of one layout, not sibling pages); inventory + a shared-component architecture that fits the no-build app. (2) THE DATA MATRIX: every datum the app shows or plans to show × have / can-get / needs-a-plan (windows, sliders, stories, facts, grapes, pairings — including FOOD pairings stored someday-filterable). (3) THE CANONICAL DATA-MODEL DOC: LWIN row schema + resolve response + record envelope + §3.4 schema in one reference with a per-attribute plan. (4) LAYOUT-VS-DATA RANKING: which screens assumed curated data; worst mismatches first. Plus the genuinely new workstream the plan never carried: **BOTTLE IMAGERY at scale** (LWIN has no images; rights-clean photos don't exist at scale; options — house-illustrated archetypes per style, user photos, hybrid, paid feeds — need a costed writeup for Ed's verdict). Output = findings docs + C-numbered questions (continue from v2's C1–C9); verdicts fold back into THIS plan next — S3's scope will grow. The bug-ledger board went live the same day (docs/bug-ledger.md + /park + /handoff skills; B-002–B-017 filed from device round 1); fix rounds sequence AFTER the audit, independent papercuts excepted. Search stays far-future but its seams stay in mind (the FTS index is one). Kickoff: docs/handoffs/2026-07-19-reckoning-audit.md. |
| D21 | Jul 19 | **S2 BUILT (capture + identify — this sprint's as-built log; Ed's device pass pending).** LWIN columns CONFIRMED against the real download (§8.2 resolved: 212,038 rows, all LWIN-7; filter STATUS=Live + TYPE∈{Wine, Fortified Wine} → 185,130 kept) · `scraps/build-lwin-index.js` (pure-JS xlsx unzip → SQLite FTS5, `unicode61 remove_diacritics 2` does the accent folding; artifact `api/_lwin/lwin.db.gz` ~17MB, gunzipped to /tmp at cold start; quarterly re-run) · `api/cellar-extract` (structured-outputs JSON envelope; default `claude-opus-4-8`, `CELLAR_EXTRACT_MODEL` is the §2.3 lever) + `api/cellar-resolve` (FTS + JS re-score — apostrophe folding sim-caught: "Tyrrells" vs "Tyrrell's" cost a perfect Vat 1 read 0.47 of score; exact-numeral gate keeps Vat 1 ≠ Vat 15; type/geography agreement breaks Rouge/Blanc ties; `CELLAR_MATCH_THRESHOLD` default 0.72 — a config value, §5.8) + `api/_lib/cellar-guards.js` (origin allowlist · `CELLAR_PIPELINE_DISABLED` · per-install ~10/hr + per-IP backstop + `CELLAR_DAILY_BUDGET`, durable via the `va_quota_bump` Supabase RPC — SQL in analytics.md's addendum, per-instance in-memory fallback until Ed runs it; quota infra fails OPEN, quota EXCEEDED always blocks) · the + affordance is the ONE entry (R3 native sheet — A1 honored; after a kill-switch 503 the session remembers and the + goes straight to the form) → identify STAGE (three mono steps, hanging dot, cancel = abandon+discard; HONEST LINES on offline/disabled/quota/unreadable with a "Your bottle, your words" door) → SPARSE match sheet (§5.2/D11: known facts + heuristic window + the quiet line) with the R2 decision bar — **as-built at the Pour's tuned 68px foot-pin, not the 88 the R2 letter carried from the fixed era** (probe-gated; Ed re-tunes on device) → correction screen (manual FIRST, runner-ups minus a rejected top, "THE LABEL READ · …") → duplicate prompt = the COUNT SHEET preset to count+1 (note "Already in the rack · Done adds this bottle" — DONE commits, scrim cancels; no new surface) · retention law enforced (photo kept ONLY manual+unmatched — saved at the photo-road form commit, deleted in `CellarStore.remove()`, decode-gated strip on detail) · records land identity-only; the settling shimmer is BOUNDED to a 30s landing moment (S3 owns the real settle loop — an unbounded shimmer would read broken for the weeks before S3) · LWIN attribution line ships (rack foot, both modes) · analytics per §5.8: `cellar_identify {conf, match, route, outcome}` (outcome enum: added·duplicate·corrected·manual·abandoned·retaken·error·offline·disabled·quota) + `cellar_added {method: "photo"}` at confirm · suite grew to **9/9** (T9: identify pan-membership, bar/nav self-carry, match ballast, THE DOCUMENT NEVER MOVES through the mocked flow — `__vaCellar.mockFlow()` never touches the pipeline) · probe grew to **6 steps** (match: reach 140 ≥ 100, zero `position:fixed`, bar top ≡ foot-vh−68 EXACT; the match band is flat-on-flat by design → recorded, device-only — the Approach precedent) · golden set v0 scaffolded (`scraps/golden-set/` + runner, .vercelignore'd — Ed's ~10 photos are checkpoint 1). **App-level calls for Ed's S2 review** (no canvas board ruled): the honest-line copy, the sparse line "Its story arrives once it settles in", the dup-note copy, 68-vs-88 bar seat, the bounded shimmer. **Ed's setup steps:** `ANTHROPIC_API_KEY` in Vercel env (until set, extract answers 503 and the app is manual-only by design) · run the va_quota SQL (analytics.md) · photograph the golden set. `?cellar-seed` retired from the docs (the function stays for harnesses). |
